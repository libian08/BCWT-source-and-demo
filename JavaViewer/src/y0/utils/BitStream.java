...
public class BitStream {
    ...
    public void allocate(int capacityInBit) {
        ...
        capacityGrowthStep = capacityInByte >> 1; // half.
    }
    ...
    public int skipInBit(int bits) {
        return setPosInBit(getPosInBit() + bits);
    }
    ...
    public void dump(DataOutput output) throws IOException {
        output.write(buf, 0, countInByte);
    }

    public void dump(DataOutput output, int offset, int len) throws IOException {
        output.write(buf, offset, len);
    }
    
    public void load(DataInput input) throws IOException {
        load(input, 0, capacityInByte);
    }
    
    public void load(DataInput input, int offset, int len) throws IOException {
    	if (offset + len > capacityInByte) {
    		allocate((offset + len) << 3); // * 8
    	}
