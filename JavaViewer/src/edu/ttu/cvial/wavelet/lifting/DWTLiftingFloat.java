 ...
 * @version 1.0.0
 */
public abstract class DWTLiftingFloat extends DWT {
     ...
     *            processed.)
     */
    protected abstract void forwardTransStep(float[] X, int N);
     ...
     *            processed.)
     */
    protected abstract void inverseTransStep(float[] X, int N);

    /** Creates a new instance of LiftingBase */
    public DWTLiftingFloat() {
    }
     ...
     *            processed.)
     */
    protected final void initStep(float[] X, int N) {
        this.X = X;
        this.N = N;
        N_minus_1 = N - 1;
        halfN = N >> 1;
     ...
     *            processed.)
     */
    private final void split(final float[] X, final int N) {
     ...
     *            processed.)
     */
    private final void merge(final float[] X, final int N) {
        float tempX[] = new float[N];
        final int oneHalf = N >> 1;
        int i;
        for (i = 0; i < oneHalf; i++) {
     ...
     */
    public float[] forwardTrans(float[] data, int maxLevel) {
     ...
     * @return Reference to the input/transformed data
     */
    public float[] forwardTrans(float[] data, int maxLevel, int minLevel) {
     ...
     * @return Reference to the input/transformed data
     */
    public float[] forwardTrans(float[] data, int maxLevel, int minLevel, int N) {
    ...
    public float[][] forwardTrans(float[][] data, int maxLevel, int minLevel,
        ...
        float[] data1D = new float[rows > cols ? rows : cols];
        int i, j, a;
        int oneHalf;
        float[] thisRow;
    ...
    public float[][] forwardTransFast(float[][] data, int maxLevel,
            int minLevel, int rows, int cols) {
        rows = rows >> (minLevel - 1);
        cols = cols >> (minLevel - 1);
        int maxLen = rows > cols ? rows : cols;
        float[] data1D0 = new float[maxLen];
        float[] data1D1 = new float[maxLen];
        float[] data1D2 = new float[maxLen];
        float[] data1D3 = new float[maxLen];
        int r, c, o, e;
        float[] thisRow;
    ...
    public short[][][][] forwardTransSpecial(float[][] data, int maxLevel,
            int minLevel, int rows, int cols) {
     ...
     * @return Reference to the input/transformed data
     */
    public float[] inverseTrans(float[] data, int maxLevel) {
     ...
     * @return Reference to the input/transformed data
     */
    public float[] inverseTrans(float[] data, int maxLevel, int minLevel) {
     ...
     */
    public float[] inverseTrans(float[] data, int maxLevel, int minLevel, int N) {
        N = N >> (maxLevel - 1);
    ...
    public float[][] inverseTrans(float[][] data, int maxLevel, int minLevel,
            int rows, int cols) {
        float[] data1D = new float[(rows > cols ? rows : cols) >> (minLevel - 1)];
        int i, j, a;
        int oneHalf;
        rows >>= maxLevel - 1;
        cols >>= maxLevel - 1;
    ...
    public float[][] inverseTransROI(float[][] data, int maxLevel, int minLevel,
    ...
    private void inverseTransOneRow(float[][] data, int cols, float[] data1D, int i) {
        int oneHalf;
        inverseTransStep(data[i], cols);
        float[] thisRow = data[i];
        oneHalf = cols >> 1;
    ...
    private void inverseTransOneColumn(float[][] data, int rows, float[] data1D, int j) {
        int oneHalf;
        for (int i = 0; i < rows; i++)
            data1D[i] = data[i][j];
        inverseTransStep(data1D, rows);
        ...
        // Merge the column and copy it back to data.
        oneHalf = rows >> 1;
        for (int i = 0, a = 0; i < oneHalf; i++) {
            data[a++][j] = data1D[i];
            data[a++][j] = data1D[oneHalf + i];
        }
    }

    public float[][] inverseTransFast(float[][] data, int maxLevel, int minLevel
            , int rows, int cols) {
